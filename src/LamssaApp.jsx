...
function PaymentMethodMark({ id, isAr }){
  const alt = {
    visa:isAr?'فيزا':'Visa', mastercard:isAr?'ماستركارد':'Mastercard', applePay:isAr?'أبل باي':'Apple Pay', googlePay:isAr?'جوجل باي':'Google Pay',
    tabby:isAr?'تابي':'Tabby', tamara:isAr?'تمارا':'Tamara',
  }
  if(id==='card') return <div className="flex h-8 flex-wrap items-center gap-1.5 rounded-[10px] bg-white px-2"><img src={`${PAYMENT_ASSET_BASE}/visa.svg`} alt={alt.visa} className="h-[13px] w-auto"/><img src={`${PAYMENT_ASSET_BASE}/mastercard.svg`} alt={alt.mastercard} className="h-[15px] w-auto"/><img src={`${PAYMENT_ASSET_BASE}/apple-pay.svg`} alt={alt.applePay} className="h-[16px] w-auto"/><img src={`${PAYMENT_ASSET_BASE}/google-pay.svg`} alt={alt.googlePay} className="h-[16px] w-auto"/></div>
...
function Checkout(){
  ...
  const paymentMethods = isAr ? [
    ...
    ['card','بطاقة / أبل باي / جوجل باي','فيزا، ماستركارد، أبل باي، جوجل باي'],
    ['tabby','تابي','قسّمها على ٤ دفعات بدون فوائد'],
    ['tamara','تمارا','ادفع لاحقاً أو قسّمها بسهولة'],
  ] : [
    ['cod','Cash on Delivery','Pay in cash when your order arrives'],
    ['card','Card / Apple Pay / Google Pay','Visa, Mastercard, Apple Pay, Google Pay'],
    ['tabby','Tabby','Split in 4 interest-free payments'],
    ['tamara','Tamara','Pay later or split it easily'],
  ]
  ...
  function buildWhatsAppMessage(ref){
    ...
    const payLabels = { cod:isAr?'الدفع عند الاستلام':'Cash on Delivery', card:isAr?'بطاقة / أبل باي / جوجل باي':'Card / Apple Pay / Google Pay', tabby:'Tabby', tamara:'Tamara' }
    lines.push(`${isAr?'الدفع':'Payment'}: ${payLabels[payment] || payment}`)
    lines.push('')
  ...
  if(done) return <main className="bg-[#ffffff] py-16 md:py-24"><div className="container-basic"><div className="max-w-2xl mx-auto rounded-[34px] border border-[#f0d4dc] bg-white p-8 md:p-12 text-center shadow-[0_30px_90px_rgba(212,86,122,0.1)]"><div className="mx-auto mb-6 grid h-16 w-16 place-items-center rounded-full bg-[#fff0f3] text-[#d4567a]"><CheckCircle2 size={38}/></div><p className="mb-3 text-[11px] uppercase tracking-[.24em] text-[#8c6b74]">{isAr?'تم تأكيد الطلب':'Order confirmed'}</p><h1 className="section-title mb-5">{isAr?'شكراً لك 💗':'Thank you 💗'}</h1><p className="text-lg mb-2">{isAr?'رقم الطلب':'Order ref'}: <b>{done.ref}</b></p><p className="mb-7 text-[#6f6b66]">{isAr?'الإجمالي':'Total'}: <b className="text-[#181818]">{fmt(done.total)}</b></p><div className="rounded-[22px] bg-[#fff0f3] p-5 text-sm leading-7 text-[#6b545a]">{isAr?'استلمنا طلبك بنجاح! بنتواصل معك قريباً لتأكيد التوصيل. توصيل الإمارات خلال ٢٤ ساعة والخليج ٣-٤ أيام.':'We received your order! We will contact you shortly to confirm delivery. UAE delivery within 24h, Gulf in 3-4 days.'}</div><div className="mt-7 flex flex-col gap-3"><a href={`https://wa.me/971567277289?text=${encodeURIComponent((isAr?`مرحباً، عندي سؤال بخصوص طلبي رقم ${done.ref}`:`Hi, I have a question about my order ${done.ref}`))}`} target="_blank" rel="noreferrer" className="flex h-[52px] w-full items-center justify-center gap-2 rounded-full bg-[#25D366] text-[13px] font-semibold uppercase tracking-[.14em] text-white transition hover:brightness-105"><WhatsAppIcon className="h-5 w-5"/> {isAr?'تواصل معنا عن الطلب':'Contact us about your order'}</a><Link to="/" className="flex h-[52px] w-full items-center justify-center rounded-full border border-[#d4567a] bg-white text-[13px] font-semibold uppercase tracking-[.14em] text-[#b8435f] transition hover:bg-[#fff0f3]">{t('continueShopping')}</Link></div></div></div></main>
  ...
  return <main className="bg-[#ffffff] py-10 md:py-16"><div className="container-basic">
    <div className="mb-8 md:mb-12 flex flex-col gap-5 md:flex-row md:items-end md:justify-between"><div><p className="mb-3 text-[11px] uppercase tracking-[.26em] text-[#8c6b74]">{isAr?'دفع آمن':'Secure checkout'}</p><h1 className="text-[30px] md:text-[44px] font-medium uppercase tracking-[.08em]">{t('checkout')}</h1><p className="mt-3 max-w-xl text-sm leading-7 text-[#706c67]">{isAr?'املأ بياناتك لإتمام طلبك. الدفع عند الاستلام متاح داخل الإمارات.':'Fill in your details to place your order. Cash on delivery available in the UAE.'}</p></div></div>
